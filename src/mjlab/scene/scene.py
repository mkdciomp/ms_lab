...
class Scene:
  ...
  def to_zip(self, path: Path) -> None:
    """Deprecated. Use ``write(output_dir, as_zip=True)`` instead."""
    warnings.warn(
      "Scene.to_zip() is deprecated. Use Scene.write(path, as_zip=True).",
      DeprecationWarning,
      stacklevel=2,
    )
    self.write(path, as_zip=True)
